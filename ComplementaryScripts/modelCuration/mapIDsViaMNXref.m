...
%load MNXref data structure from raven repository
%since right now, metanet reference only exist in the branch:
...
%Check if this branch is add_MetaNetX
    currentBranch = git('rev-parse --abbrev-ref HEAD');
    if ~strcmp(currentBranch,'add_MetaNetX')
        git checkout add_MetaNetX 
        currentBranch = git('rev-parse --abbrev-ref HEAD');
        if ~strcmp(currentBranch,'add_MetaNetX')
            git checkout -b add_MetaNetX origin/feat/add_MetaNetX
            currentBranch = git('rev-parse --abbrev-ref HEAD');
            if ~strcmp(currentBranch,'add_MetaNetX')
